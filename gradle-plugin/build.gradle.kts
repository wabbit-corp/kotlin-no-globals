...
// managed-integrity-v1: SxM5jp1yqVk
//
...
abstract class GenerateNoGlobalsGradlePluginVersion : DefaultTask() {
    @get:Input
    ...
    fun generate() {
        ...
        file.writeText(
            ...
            internal const val NO_GLOBALS_GRADLE_PLUGIN_VERSION: String = "${compilerPluginVersion.get()}"
            """.trimIndent(),
        )
    }
}
...
val configuredBaseVersionString = providers.gradleProperty("projectVersion").orElse("0.1.1").get()
...
mavenPublishing {
    coordinates("one.wabbit", "kotlin-no-globals-gradle-plugin", project.version.toString())
    ...
    pom {
        ...
        scm {
            url.set("https://github.com/wabbit-corp/kotlin-no-globals")
            connection.set("scm:git:git://github.com/wabbit-corp/kotlin-no-globals.git")
            developerConnection.set("scm:git:ssh://git@github.com/wabbit-corp/kotlin-no-globals.git")
        }
    }
}
...
publishing {
    publications.withType<org.gradle.api.publish.maven.MavenPublication>().configureEach {
        if (name == "pluginMaven") {
            artifactId = "kotlin-no-globals-gradle-plugin"
        }
    }
}
...
val generateCompilerGradlePluginVersion by tasks.registering(GenerateNoGlobalsGradlePluginVersion::class) {
    compilerPluginVersion.set(configuredBaseVersionString)
    outputFile.set(generatedVersionSourceDirectory.map { directory ->
        directory.file("one/wabbit/noglobals/gradle/NoGlobalsGradlePluginVersion.kt")
    })
}
...
kover {
    ...
    //             // title.set("kotlin-no-globals-gradle-plugin Code Coverage") // Uncomment to customize title
    //         }
    //         xml {
...
plugins.withId("org.jetbrains.dokka") {
    afterEvaluate {
        extensions.findByType(DokkaExtension::class)?.apply {
            moduleName.set("kotlin-no-globals-gradle-plugin")
            ...
            dokkaSourceSets.named("main").configure {
                ...
                sourceLink {
                    ...
                    remoteUrl("https://github.com/wabbit-corp/kotlin-no-globals/tree/master/gradle-plugin/src/main/kotlin")
