...
internal data class NoGlobalsIdeSupportResult(
    val scan: NoGlobalsCompilerPluginScan,
...
internal object NoGlobalsIdeSupportCoordinator {
    private val logger = Logger.getInstance(NoGlobalsIdeSupportCoordinator::class.java)
    internal var gradleImportRequester: NoGlobalsGradleImportRequester = DefaultNoGlobalsGradleImportRequester
    internal var analysisRestarter: NoGlobalsAnalysisRestarter = DefaultNoGlobalsAnalysisRestarter

    fun enableIfNeeded(project: Project, userInitiated: Boolean): NoGlobalsIdeSupportResult {
        val scan = NoGlobalsCompilerPluginDetector.scan(project)
        ...
        val enablePlan =
            NoGlobalsIdeSupportDecision.evaluate(
                scan = scan,
        ...
        if (!scan.hasMatches) {
            if (userInitiated) {
                notify(
                    ...
                    title = "No-Globals IDE support",
                    content =
                        "No imported Kotlin compiler arguments or Gradle build files reference kotlin-no-globals-plugin or one.wabbit.no-globals.",
                )
            }
            return NoGlobalsIdeSupportResult(
                scan = scan,
                projectTrusted = trusted,
        ...
        if (!trusted) {
            notify(
                ...
                title = "No-Globals IDE support is waiting for trust",
                content =
                    "kotlin-no-globals-plugin was detected, but IntelliJ will not load external compiler plugins until the project is trusted.",
            )
            return NoGlobalsIdeSupportResult(
                scan = scan,
        ...
        if (registryUpdated || gradleImportRequested || userInitiated) {
            notify(
                project = project,
                type = NotificationType.INFORMATION,
                title = "No-Globals IDE support is active",
                content =
                    buildEnabledMessage(
                        scan = scan,
    ...
    internal fun buildEnabledMessage(
        scan: NoGlobalsCompilerPluginScan,
        ...
        return "$prefix Detected kotlin-no-globals-plugin in $ownerSummary.$refreshMessage"
    }
    ...
    private fun notify(
        ...
        NotificationGroupManager.getInstance()
            .getNotificationGroup("NoGlobalsIdeSupport")
            .createNotification(title, content, type)
            .notify(project)
    }
}

internal data class NoGlobalsIdeSupportDecision(
    ...
    companion object {
        ...
        ): NoGlobalsIdeSupportDecision {
            if (!scan.hasMatches || !projectTrusted) {
                return NoGlobalsIdeSupportDecision(
...
internal interface NoGlobalsGradleImportRequester {
    fun requestImport(project: Project): Boolean
}

internal object DefaultNoGlobalsGradleImportRequester : NoGlobalsGradleImportRequester {
    override fun requestImport(project: Project): Boolean {
        ...
        return runCatching {
            ...
            Logger.getInstance(NoGlobalsIdeSupportCoordinator::class.java).warn(
...
internal interface NoGlobalsAnalysisRestarter {
    fun restart(project: Project)
}

internal object DefaultNoGlobalsAnalysisRestarter : NoGlobalsAnalysisRestarter {
